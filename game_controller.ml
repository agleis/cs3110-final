...
let rec initialize_state ai_list name_list deck =
	...
	{pool=[]; prs=p_states; phase=Pass; round_num=0;
		last_human_player=next_human}
	...
	match ps with
		| h::t -> begin
			...
			if is_ai then begin
				...
				let new_players = List.map (fun x -> if x.p_num=h.p_num
									then new_p_state else x) st.prs in
			...
			let to_pass = get_human_cards_to_pass
				{st with last_human_player=h.p_num} h in
			let new_hand = remove_cards h.hand to_pass in
			let new_p_state = {h with hand=new_hand} in
			let new_players = List.map (fun x -> if x.p_num=h.p_num
				then new_p_state else x) st.prs in
			let () = draw_board
			{st with last_human_player=h.p_num; prs=new_players} h in
			to_pass::(process_players_trades
				{st with last_human_player=h.p_num; prs=new_players} t)
			end
		end
		| [] -> []
	...
	match ps with
		| h::t-> begin
			...
			if is_ai then begin
				...
				let card_to_play =
					Ai_controller.guess_turn h pool_cards data in
				let has_2clubs = List.exists
					(fun x-> x={suit=Club; value=2}) h.hand in
				let new_hand = remove_cards h.hand [card_to_play] in
				let new_p_state = {h with hand=new_hand} in
				let new_players = List.map (fun x -> if x.p_num=h.p_num
					then new_p_state else x) st.prs in
				let () = draw_board {st with prs=new_players;
					pool=((card_to_play,h.p_num)::st.pool)} h in
				let () = Unix.sleep 1 in
				let () = fix_ai_data card_to_play
					(get_ordered_p_states st.prs) data in
				let ns = {st with prs=new_players;
					pool=((card_to_play,h.p_num)::st.pool)} in
				...
				let () = if multi_player then switch_player h.name else () in
				let () = draw_board {st with last_human_player=h.p_num} h in
				let card_to_play = get_human_card_to_play
					{st with last_human_player=h.p_num} h data in
				let new_hand = remove_cards h.hand [card_to_play] in
				let new_p_state = {h with hand=new_hand} in
				let new_players = List.map (fun x -> if x.p_num=h.p_num
					then new_p_state else x) st.prs in
...
let rec repl st (data:stored_data) =
	...
	begin
		if n_st.phase=Pass then
			...
			let () = fix_ai_data_suits
				(get_ordered_p_states n_state.prs) data.players in
			let reorder_players = reorder_players_2clubs n_state.prs [] in
			let next_human = find_first_human reorder_players in
			repl {n_state with prs=reorder_players;
				last_human_player=next_human} data
		else
...
and reflush_round (st:game_state) data =
	let point_list = List.map (fun x-> x.round_pts)
		(get_ordered_p_states st.prs) in
	...
	let total_points = List.map (fun x-> x.game_points)
		(get_ordered_p_states new_players) in
	...
	let f_players = List.map2
		(fun p h -> {p with hand=h; round_pts=0}) new_players hands in
